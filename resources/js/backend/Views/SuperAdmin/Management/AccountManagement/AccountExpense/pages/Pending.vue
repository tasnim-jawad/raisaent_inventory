          ...
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-12 col-md-3 mb-2 mb-md-0">
                <h5 class="text-capitalize mb-0">All Pending Vouchers</h5>
              </div>
              ...
              <table class="table table-hover text-center table-bordered">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td>{{ item.account_category?.title.substr(0, 50) }}</td>
                    <td>{{ item.title.substr(0, 50) }}</td>
                    <td>{{ item.amount }}</td>
                    <td>
    ...
    >
      <div class="off_canvas_body">
        ...
        <div class="data_content">
          ...
          <div class="filter_item">
            <label for="sort_by_col">Employee</label
            ><label
              for="sort_by_col"
              class="text-capitalize d-block date_custom_control"
            >
              <select v-model="employee_id" class="form-control">
                <option v-for="col in employees" :key="col" :value="col.id">
                  {{ col.name }}
                </option>
              </select>
            </label>
...
export default {
  ...
  created: async function () {
    this.set_status("pending_voucher");
    await this.get_all();
  ...
  methods: {
    ...
    get_employees: async function () {
      let response = await axios.get("users?employee_list=1&get_all=1");
    ...
    updateStatus: async function (event, item) {
      let conf = await window.s_confirm(
      ...
      let is_approved = event.target.checked; // Get the checked state

      try {
        let response = await axios.post(
          "account-expenses/update-status", // Assuming you're posting data to this endpoint
          {
            slug: item.slug,
        ...
        window.s_warning("An error occurred while updating the status.");
      }
    },
  },
  computed: {
